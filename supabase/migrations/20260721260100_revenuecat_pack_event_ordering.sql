-- Consumable pack purchases are independent grants. Do not discard a valid
-- pack event merely because its provider timestamp is older than a separate
-- subscription-state event delivered first.
...
begin
  ...
  -- Process consumable grants before subscription-state ordering. The unique
  -- provider_event_id remains the idempotency fence for the grant itself.
