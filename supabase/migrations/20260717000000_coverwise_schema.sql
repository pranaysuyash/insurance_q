...
-- The API uses the server-only service_role client. RLS remains enabled for
-- defense in depth, but the role still needs table and identity-sequence
-- privileges for PostgREST to execute the repository operations.
grant select, insert, update, delete on table public.documents, public.document_chunks
  to service_role;
