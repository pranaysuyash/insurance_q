-- A provider REFUND_REVERSED event means a prior refund was reversed. Keep
-- the event authoritative, but restore access only while its expiry is future.
-- This replaces the canonical ledger function without rewriting historical
-- migrations that may already have been applied to another environment.
