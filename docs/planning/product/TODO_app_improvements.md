...
- [ ] **P0-01: Fix RAG Service Error in Q&A**
  - Fix the error: "Error communicating with RAG service: {"detail":"An unexpected error occurred during query processing: 'result'}"}"
  - Implement proper error handling in the service.py file with the recent compatibility fixes
...
- [ ] **P1-02: Prevent Duplicate Document Uploads**
  - Implement document fingerprinting to detect duplicates
  - Add warning when attempting to upload a duplicate
  - Offer "replace" option for duplicates
  - Keep a log of upload attempts for troubleshooting
...
- [ ] **P1-04: Fix Default Document Selection in Q&A**
  - Default to most recently viewed document when entering Q&A screen
  - If no documents exist, provide clear guidance to upload one
  - Use filename as fallback when document type is unknown
  - Pre-select the only document if just one exists
...
- [ ] **P2-02: Improve Document Limit Messaging**
  - Rephrase "oldest will be removed" to be less alarming
