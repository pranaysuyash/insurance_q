...
| BR-04 | Authentication and guest-to-account continuity | Tier 2 local contracts; current Tier 3 rerun blocked by unavailable local services | Anonymous upload → email and Google account conversion → restart → source, evidence and Q&A readback; no orphaned workspace | Engineering + Supabase account owner |
| BR-05 | Tenant isolation, storage and deletion | Tier 2 local owner-scope/substrate contract; static Storage/RLS contract and live verification pending | Two-principal access-denial test, storage-owner policy verification, deletion request through durable worker, post-delete object/artifact audit | Engineering + Supabase account owner |
