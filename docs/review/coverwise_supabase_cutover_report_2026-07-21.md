...
### Current remote recheck (2026-07-21)

The service-role REST probe still finds `public.model_run_results` absent with
PostgREST error `PGRST205`; the other required schema tables remain queryable.
The repository is not currently linked to the Supabase project, and the
available experimental access token was rejected by the installed CLI. The
earlier remote-application claim is therefore not current proof for this
table; it remains an explicit migration gate until a dry-run and post-apply
schema probe pass.

### Current verification addendum (2026-07-21)

The later worktree verification supersedes those earlier counts for current
state: the backend suite passed 359 tests with 1 intentional deployment-gated
skip, and the deterministic Flutter suite passed 596 tests with one worker.
...
The non-mutating schema verifier was rerun against the configured project and
returned exit code 0. All required tables, including `public.model_run_results`,
were queryable; Auth returned HTTP 200 with email enabled and anonymous users
disabled. This supersedes the older absence observation. Migration-history
parity, publishable-key protected-table checks beyond the probe, and deployed
application E2E remain release gates.

### Expanded remote object audit (2026-07-21)

The read-only parity audit found no missing repository-declared tables,
functions, indexes, triggers, columns, policies, or extensions; all 46 remote
public tables and the six required reservation RPCs are present. It also found
nine remote migration-ledger version IDs generated by the Management API that
do not match repository migration filenames. This leaves schema/object parity
green but migration-history baseline reconciliation open.
