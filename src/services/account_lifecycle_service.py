...
def process_deletion(request_id: str, account_uid: str) -> None:
    ...
    if not started.data:
        current = client.table("account_deletion_requests").select("status").eq("id", request_id).limit(1).execute()
        ...
        client.table("account_deletion_requests").update({
            "status": "running",
            "started_at": datetime.now(timezone.utc).isoformat(),
        }).eq("id", request_id).eq("status", "failed").execute()
    stage_state: dict[str, Any] = {"storage": {"deleted": 0, "failed": 0}, "documents": 0, "auth": False}
    try:
        ...
        from src.services.artifact_registry import mark_owner_deleted
        stage_state["artifacts"] = mark_owner_deleted(account_uid)
