...
def fix_response_format(response_data, query=None):
    ...
    try:
            ...
            if "result" not in response_data and "answer" in response_data:
                return {
                    ...
                    "result": {
                        ...
                        "sources": response_data.get("sources", []),
                        "query": query,
                        "embedding_model_used": response_data.get(
        ...
        if "status" not in response_data or "result" not in response_data:
            return {
                "status": "error",
                "error": "Unable to repair malformed cache entry",
            }
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Don't modify data, just report"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Show detailed information"
    )
    parser.add_argument(
        "--pattern", default="rag:query:*", help="Redis key pattern to scan"
    )
