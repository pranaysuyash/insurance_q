...
def process_document_via_api(file_path):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Test processing failed: {response.status_code} - {response.text}")
            return False
    ...
    try:
        ...
        for file_path in document_files:
            try:
                ...
                import asyncio
                result = asyncio.run(doc_service.process_document_full(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get('sources'):
                print("First source preview:", result['sources'][0][:100] + "..." if len(result['sources'][0]) > 100 else result['sources'][0])
        else:
