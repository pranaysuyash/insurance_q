...
async def test_embedding_fallback():
    ...
    parser = argparse.ArgumentParser(
        description="Test RAG pipeline embedding fallback mechanism"
    )
    parser.add_argument(
        "--openai-first",
        action="store_true",
        help="Use OpenAI first (default is HF first)",
    )
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument(
        "--chat-model",
        default=os.getenv("OPENAI_CHAT_MODEL", "gpt-4.1-nano"),
    ...
    parser.add_argument(
        "--small",
        action="store_true",
        help="Use a smaller test set (3 texts instead of 5)",
    )
    parser.add_argument(
        "--model-sequence",
        action="store_true",
    ...
    logger.info(
        f"Testing with {'OpenAI' if use_openai_first else 'Hugging Face'} as primary embedding model"
    )
    ...
    logger.info(
        f"Will test the following embedding models: {', '.join(embedding_models)}"
    )
    ...
    for embedding_model in embedding_models:
        ...
        logger.info(
            f"Attempting to ingest document {test_doc_id} with {len(test_blocks)} blocks"
        )
        try:
            ...
            logger.info(
                "Testing single embedding generation before batch processing..."
            )
            await pipeline._generate_embeddings_with_fallback(
                ["Test connection to embedding API"]
            )
            logger.info(
                f"Single test embedding successful with model: {pipeline.active_embedding_model}"
            )

            # Process document
            ingest_result = await pipeline.ingest_document_data(
                test_doc_id, test_blocks
            )
            ...
            if (
                ...
                and ingest_result.get("points_added", 0) > 0
            ):
                logger.info("Testing search...")
                # Add a small delay before querying
                time.sleep(2)
                query_result = await pipeline.query_rag(
            ...
            logger.error(
                f"Test failed with model {embedding_model}: {e}", exc_info=True
            )
            ...
            try:
                stats = await pipeline.get_embedding_stats()
                logger.info(
                    f"Final embedding stats after failure: {json.dumps(stats, indent=2)}"
                )
            except Exception:
                pass
