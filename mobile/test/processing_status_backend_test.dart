...
void main() {
  group('stageFromState — backend granular stages', () {
    ...
    test('maps backend "extracting_policy_data" to extraction', () {
      expect(stageFromState('extracting_policy_data'), ProcessingStage.extraction);
    });
  ...
  group('ProcessingStage properties', () {
    ...
    test('all stages have non-empty labels', () {
      for (final stage in ProcessingStage.values) {
        expect(stage.label.isNotEmpty, true, reason: 'Stage ${stage.name} has empty label');
      }
    });

    test('all stages have non-empty descriptions', () {
      for (final stage in ProcessingStage.values) {
        expect(stage.description.isNotEmpty, true, reason: 'Stage ${stage.name} has empty description');
      }
    });
  });
  ...
  group('Stage progression logic', () {
    test('complete is ahead of all processing stages', () {
      expect(ProcessingStage.complete.step, greaterThan(ProcessingStage.indexing.step));
      expect(ProcessingStage.complete.step, greaterThan(ProcessingStage.classification.step));
      expect(ProcessingStage.complete.step, greaterThan(ProcessingStage.extraction.step));
      expect(ProcessingStage.complete.step, greaterThan(ProcessingStage.processing.step));
      expect(ProcessingStage.complete.step, greaterThan(ProcessingStage.received.step));
    });

    test('failed is at same step as complete', () {
      expect(ProcessingStage.failed.step, equals(ProcessingStage.complete.step));
    });

    test('stages are in correct order', () {
      expect(ProcessingStage.received.step, lessThan(ProcessingStage.processing.step));
      expect(ProcessingStage.processing.step, lessThan(ProcessingStage.extraction.step));
      expect(ProcessingStage.extraction.step, lessThan(ProcessingStage.classification.step));
      expect(ProcessingStage.classification.step, lessThan(ProcessingStage.indexing.step));
      expect(ProcessingStage.indexing.step, lessThan(ProcessingStage.complete.step));
    });
  });
}
