...
class _Table:
    ...
    def execute(self):
        return SimpleNamespace(
            data=self.rows or ([self.upserted] if self.upserted else [])
        )
...
class _Client:
    ...
    def rpc(self, _name, params):
        ...
        return SimpleNamespace(
            execute=lambda: SimpleNamespace(data={"status": "processed"})
        )
...
def test_remote_qa_pack_balance_uses_server_rpc():
    ...
    client.rpc = lambda name, params: SimpleNamespace(
        execute=lambda: SimpleNamespace(
            data={
                "packs": [
                    {"product_id": "coverwise_qa_starter", "questions_remaining": 5}
                ],
...
def test_pack_balance_migration_is_server_only_and_expiry_filtered():
    sql = Path(
        "supabase/migrations/20260721260200_qa_pack_balance_readback.sql"
    ).read_text()
...
def test_identity_claim_migration_moves_pack_grants_with_documents():
    ...
    assert (
        "grant execute on function public.claim_anonymous_documents(text, text)" in sql
    )
...
def test_refund_reversal_migration_restores_only_a_current_entitlement():
    sql = Path(
        "supabase/migrations/20260724000000_revenuecat_refund_reversal_semantics.sql"
    ).read_text()

    assert "'REFUND_REVERSED'" in sql
    assert "then p_expires_at is null or p_expires_at > now()" in sql
